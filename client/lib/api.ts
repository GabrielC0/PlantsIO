...
const BASE =
  process.env.NEXT_PUBLIC_API_URL || "http://localhost:45024/plantsio";
...
export const api = {
  // ── System ──────────────────────────────────────────────────────
  getSystemState: () => request<SystemState>("/api/v1/system/state"),
  ...
  pumpOff: () =>
    request<{
      ...
      durationSeconds: number;
      volumeLiters: number;
    }>("/api/v1/pump/off", { method: "POST", body: "{}" }),

  // ── Mode ────────────────────────────────────────────────────────
  setMode: (autoMode: boolean) =>
    request<{ autoMode: boolean; updatedAt: string }>("/api/v1/mode", {
      method: "PUT",
      body: JSON.stringify({ autoMode }),
    }),

  // ── Schedules ───────────────────────────────────────────────────
  getSchedules: () => request<Schedule[]>("/api/v1/schedules"),
  ...
  deleteSchedule: (id: string) =>
    request<{ deleted: boolean }>(`/api/v1/schedules/${id}`, {
      method: "DELETE",
    }),
  ...
  // ── Stats ────────────────────────────────────────────────────────
  getWeeklyStats: () => request<WeeklyStat[]>("/api/v1/stats/weekly"),
